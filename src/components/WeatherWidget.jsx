...
export default function WeatherWidget() {
  ...
  if (state.status === 'loading') {
    return (
      <div className="flex items-center gap-2.5 text-text-tertiary text-sm">
  ...
  if (state.status === 'denied' || state.status === 'unavailable') {
    return (
      <div className="flex items-start gap-2.5 text-text-tertiary text-sm max-w-[15rem]">
        <LocationIcon size={16} className="shrink-0 mt-0.5" />
  ...
  return (
    <div>
      <p className="kicker mb-2">Weer vandaag</p>
      <div className="flex items-center gap-3">
        <Icon size={26} className="text-accent-soft-text shrink-0" />
        <div className="leading-tight">
          <p className="font-serif font-semibold text-text-primary text-2xl">
            {state.temp}°<span className="byline font-sans ml-1.5">{label}</span>
          </p>
