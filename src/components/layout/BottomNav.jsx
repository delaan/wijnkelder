  ...
  return (
    <div className="md:hidden fixed inset-0 z-modal flex items-end justify-center" role="dialog" aria-modal="true" aria-label="Meer" ref={dialogRef}>
      ...
      <div className="relative w-[calc(100%-1.5rem)] max-w-sm mb-[calc(0.75rem+env(safe-area-inset-bottom))] space-y-2 animate-slide-up">
        <div className="rounded-token-lg overflow-hidden bg-surface border border-border-strong divide-y divide-border">
          {MORE_MENU_ITEMS.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full h-14 flex items-center gap-3 px-4 text-base font-serif ${active ? 'text-accent-soft-text font-semibold' : 'text-text-primary'}`}
              >
        ...
        <button
          onClick={onClose}
          className="w-full h-14 rounded-token-lg bg-surface border border-border-strong text-base font-semibold text-accent-soft-text"
        >
          Annuleren
        </button>
      </div>
...
export default function BottomNav({ view, onNavigate, onAdd }) {
  ...
  const renderItem = (item) => {
    ...
    return (
      <button
        ...
        onClick={() => onNavigate(item.key)}
        aria-current={active ? 'page' : undefined}
        className="flex flex-col items-center justify-center gap-1 h-14 min-w-0 px-0.5 text-[9.5px] font-medium"
      >
        <span
          className={`flex items-center justify-center transition-colors duration-fast ${
            active ? 'text-accent-soft-text' : 'text-text-tertiary'
          }`}
        >
          <Icon size={18} />
        </span>
  ...
  return (
    <>
      <nav
        className="md:hidden fixed inset-x-0 bottom-0 z-nav safe-left safe-right safe-bottom border-t-[3px] border-double border-text-primary bg-surface"
        aria-label="Hoofdnavigatie"
      >
        <div>
          <div className={`grid ${onAdd ? 'grid-cols-5' : 'grid-cols-4'} px-1 pt-1`}>
            ...
            {onAdd && (
              <div className="flex items-center justify-center min-w-0">
                <button
                  ...
                  className="w-[52px] h-[52px] -mt-6 rounded-full bg-accent hover:bg-accent-hover text-accent-contrast flex items-center justify-center border-4 border-bg transition-colors duration-fast"
                >
                  <PlusIcon size={20} />
                </button>
              </div>
            ...
            >
              <span
                className={`flex items-center justify-center transition-colors duration-fast ${
                  moreActive ? 'text-accent-soft-text' : 'text-text-tertiary'
                }`}
              >
                <MoreIcon size={18} />
              </span>
