...
export function useCellarSettings(userId) {
  ...
  const uploadAvatar = async (file) => {
    const ext = file.name.split('.').pop()
    const path = `${userId}/${crypto.randomUUID()}.${ext}`
    const { error } = await supabase.storage.from('cellar-avatars').upload(path, file)
    if (error) throw error
    const { data } = supabase.storage.from('cellar-avatars').getPublicUrl(path)
    return data.publicUrl
  }
  ...
  return { settings, loading, refetch, update, uploadLogo, uploadHeroImage, uploadAvatar }
}
